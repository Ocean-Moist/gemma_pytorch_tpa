...
class TPAAttention(nn.Module):
    """
    Factor-only Tensor Product Attention (TPA).

    This module never forms full Q, K, or V arrays.
    It computes factorized attention scores and outputs from low-rank factors.
    """
    def __init__(self, config: gemma_config.GemmaConfig, attn_type: gemma_config.AttentionType):
        ...
        # Factor projections: A and B parts.
        ...
        # Factor caches for keys and values.
        self.cache_kA = None
        self.cache_kB = None
        self.cache_vA = None
        self.cache_vB = None
    ...
    def forward(
            ...
            kv_write_indices: torch.Tensor,   # indices to write into the cache
            kv_cache,                         # used for shape inference
            mask: torch.Tensor,               # [batch, 1, q_len, kv_seq_len]
            local_mask: torch.Tensor = None,  # optional local mask
    ) -> torch.Tensor:
        bsz, seq_len, _ = hidden_states.shape
        device = hidden_states.device

        # Compute A and B factors for Q, K, and V.
        A_q = self.W_A_q(hidden_states).view(bsz, seq_len, self.num_heads, self.q_rank)
        A_k = self.W_A_k(hidden_states).view(bsz, seq_len, self.num_kv_heads, self.k_rank)
        A_v = self.W_A_v(hidden_states).view(bsz, seq_len, self.num_kv_heads, self.v_rank)

        B_q = self.W_B_q(hidden_states).view(bsz, seq_len, self.q_rank, self.head_dim)
        B_k = self.W_B_k(hidden_states).view(bsz, seq_len, self.k_rank, self.head_dim)
        B_v = self.W_B_v(hidden_states).view(bsz, seq_len, self.v_rank, self.head_dim)
        ...
        A_k = self.cache_kA[:bsz, :cache_len]  # [b, kv_seq, num_kv_heads, k_rank]
        A_v = self.cache_vA[:bsz, :cache_len]  # [b, kv_seq, num_kv_heads, v_rank]
        ...
        B_dots = torch.einsum('bqrd,bksd->bqkrs', B_q, B_k)  # shape: [b, q_len, kv_seq, q_rank, k_rank]
        B_dots = B_dots * self.scaling

        # Compute attention logits via factorized A factors:
        attn_weights = torch.einsum('bqhr,bkhs,bqkrs->bhqk', A_q, A_k, B_dots)
        # attn_weights shape: [b, num_heads, q_len, kv_seq_len]
        ...
        # If local sliding window is used, choose the local mask.
        ...
        # --- Ensure the mask shape matches ---
        if mask is not None:
            # Expected shape: [b, 1, q_len, kv_seq_len]
            if mask.shape[-2:] != attn_weights.shape[-2:]:
                # If mask is transposed, fix it.
                mask = mask.transpose(-2, -1)
            attn_weights = attn_weights + mask

        attn_probs = F.softmax(attn_weights.float(), dim=-1).type_as(hidden_states)

        # Multiply by factorized V to obtain the output.
        # A_v: [b, kv_seq, num_heads, v_rank]
        # B_v: [b, kv_seq, v_rank, head_dim]
        attn_output = torch.einsum('bhqk,bkhu,bkud->bhqd', attn_probs, A_v, B_v)
    ...
    def _apply_rotary_emb_to_B(self, B: torch.Tensor, freqs_cis: torch.Tensor):
        """
        Applies rotary position embeddings to each rank slice of B.
        B: [b, seq, rank, head_dim]
        freqs_cis: [seq, head_dim//2] (complex tensor)
        """
        ...
        for t in range(seq_len):
            freqs = freqs_cis[t]  # shape: [hd//2] (complex)
            for r in range(rank):
                b_slice = B[:, t, r, :]  # [bsz, hd]
                b_complex = torch.view_as_complex(b_slice.float().reshape(bsz, -1, 2))
                b_rotated = b_complex * freqs  # broadcast multiplication
                b_rotated = torch.view_as_real(b_rotated).reshape(bsz, hd)
